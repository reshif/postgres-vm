...
function Debugger({ scope }: { scope: Scope }) {
  ...
  const [pack, setPack] = useState<any>(null); const [error, setError] = useState<Error | null>(null);
  const run = async () => { setError(null); try { setPack(await write<any>("/v1/context", scope, { task, token_budget: 4000, as_of: asOf })); } catch (reason) { setError(reason as Error); } };
  const exportCase = async () => { if (!pack) return; const template = await read<any>("/v1/eval/case-template", scope, { pack_id: pack.pack_id }); const blob = new Blob([JSON.stringify(template, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `eval-case-${pack.pack_id}.json`; link.click(); URL.revokeObjectURL(url); };
  ...
  return <section>
    ...
    {pack && <div className="debug-stages">
      ...
      <div className="actions"><button onClick={() => void exportCase()}>Export as eval case</button><span className="muted">profile: {pack.ranking_profile} {pack.degraded ? "- lexical degraded mode" : ""}</span></div>
    </div>}
  </section>;
}
