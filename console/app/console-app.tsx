...
function Debugger({ scope }: { scope: Scope }) {
  ...
  return <section>
    ...
    {pack && <div className="debug-stages">
      ...
      <div className="actions"><button onClick={exportCase} disabled={!caseTemplate || !Object.values(labels).some(label => typeof label === "number")}>Export reviewed eval case</button><span className="muted">profile: {pack.ranking_profile} {pack.degraded ? "- lexical degraded mode" : ""}</span></div>
    </div>}
  </section>;
}
